...
$pageTitle = 'Kelola Praktikum PAR';
$current_page = 'praktikum-par';
...
<main class="admin-content">
    <div class="container-fluid px-4 py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold text-dark mb-1">Kelola Praktikum PAR</h2>
    ...
    <div class="modal-dialog">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header border-0 bg-light">
                <h5 class="modal-title fw-bold">Tambah Software</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="addSoftwareForm">
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Nama Software</label>
                        <input type="text" class="form-control" id="softwareName" placeholder="Contoh: Arduino IDE" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Link Website (Optional)</label>
                        <input type="url" class="form-control" id="softwareLink" placeholder="https://...">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">File Installer (Optional)</label>
...
function loadModules() {
    ...
    fetch(BASE_URL + '/php-native/api/praktikum/par/modules.php')
        .then(res => res.json())
        .then(data => {
                            ...
                            <tbody>
                                ${data.data.map(item => `
                                    <tr>
                                        ...
                                        <td>
                                            ${item.file ? 
                                                `<a href="<?php echo BASE_URL; ?>/public/uploads/${item.file}" target="_blank" class="text-primary">
...
function loadRegistration() {
    ...
    fetch(BASE_URL + '/php-native/api/praktikum/par/registration.php')
        .then(res => res.json())
        .then(data => {
            if (data.data && data.data.length > 0) {
                ...
                container.innerHTML = `
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Link Pendaftaran Aktif:</label>
                        <div class="input-group">
                            <input type="text" class="form-control" value="${reg.link}" readonly>
...
function loadSoftware() {
    ...
    fetch(BASE_URL + '/php-native/api/praktikum/par/software.php')
        .then(res => res.json())
        .then(data => {
            if (data.data && data.data.length > 0) {
                container.innerHTML = `
...
function deleteModule(id) {
    if (confirm('Yakin ingin menghapus modul ini?')) {
        fetch(BASE_URL + '/php-native/api/praktikum/par/modules.php?id=' + id, {
            method: 'DELETE'
        })
        .then(res => res.json())
        .then(data => {
...
function deleteSoftware(id) {
    if (confirm('Yakin ingin menghapus software ini?')) {
        fetch(BASE_URL + '/php-native/api/praktikum/par/software.php?id=' + id, {
            method: 'DELETE'
        })
        .then(res => res.json())
        .then(data => {
