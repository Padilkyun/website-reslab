...
try {
    ...
    if ($method === 'GET') {
        $stmt = $pdo->query("SELECT * FROM RangkaianListrikSoftwareReq ORDER BY id ASC");
    ...
    elseif ($method === 'POST') {
        ...
        if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            $fileName = 'rangkaian_software_' . time() . '_' . uniqid() . '.' . $ext;
        ...
        $stmt = $pdo->prepare("INSERT INTO RangkaianListrikSoftwareReq (name, link, file) VALUES (?, ?, ?)");
    ...
    elseif ($method === 'PUT') {
        ...
        $stmt = $pdo->prepare("UPDATE RangkaianListrikSoftwareReq SET name = ?, link = ? WHERE id = ?");
    ...
    elseif ($method === 'DELETE') {
        ...
        $stmt = $pdo->prepare("SELECT file FROM RangkaianListrikSoftwareReq WHERE id = ?");
        $stmt->execute([$id]);
        $software = $stmt->fetch();
        ...
        $stmt = $pdo->prepare("DELETE FROM RangkaianListrikSoftwareReq WHERE id = ?");
        $stmt->execute([$id]);
