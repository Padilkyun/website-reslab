...
try {
    ...
    if ($method === 'GET') {
        $stmt = $pdo->query("SELECT * FROM SistemTertanamSoftwareReq ORDER BY id ASC");
    ...
    elseif ($method === 'POST') {
        ...
        if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            $fileName = 'sistem_software_' . time() . '_' . uniqid() . '.' . $ext;
        ...
        $stmt = $pdo->prepare("INSERT INTO SistemTertanamSoftwareReq (name, link, file) VALUES (?, ?, ?)");
    ...
    elseif ($method === 'PUT') {
        ...
        $stmt = $pdo->prepare("UPDATE SistemTertanamSoftwareReq SET name = ?, link = ? WHERE id = ?");
    ...
    elseif ($method === 'DELETE') {
        ...
        $stmt = $pdo->prepare("SELECT file FROM SistemTertanamSoftwareReq WHERE id = ?");
        $stmt->execute([$id]);
        $software = $stmt->fetch();
        ...
        $stmt = $pdo->prepare("DELETE FROM SistemTertanamSoftwareReq WHERE id = ?");
        $stmt->execute([$id]);
